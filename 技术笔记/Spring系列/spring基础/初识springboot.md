...
+ 从spirng繁琐xml配置中脱离出来
+ 更强的扩展性，更简单的集成
+ 简化部署流程,内嵌servlet容器，war->jar的部署,独立的项目运行
省去了原始的war包解析和麻烦分离的tomcat的配置
+ 习惯优于配置的理念
+ starter模式的依赖整合
+ 通过springbootapplcation的类路径自动扫描jar进行bean的加载

#### springboot环境准备
+ jdk1.8 开发工具包
+ maven 构建工具及包管理工具
+ servlet 容器 tomcat9 jetty

#### 搭建springboot web应用三步曲
1. 创建maven项目
2. 继承starter父pom工程，引入spirng-web依赖
3. 创建启动启动类作为应用入口
4. 项目打包
